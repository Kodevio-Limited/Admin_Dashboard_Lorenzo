...
import { useGetPrivacyPolicy, useUpdatePrivacyPolicy } from '@/hooks/api/useSettings';

export default function PrivacyPolicyPage() {
  const { data, isLoading, isError, error } = useGetPrivacyPolicy();
        ...
        <div className="bg-dark-600 rounded-[8px] p-6 w-full">
          {isError && (
            <div className="mb-4 p-4 bg-red-950/40 border border-red-800/50 rounded text-red-400 text-sm">
              Failed to load privacy policy: {error?.message || 'Unknown error'}
            </div>
          )}
