...
export default function MediaCard({ media, onEdit }: MediaCardProps) {
  const fileName = media.attachment?.originalName || media.fileName || `Media #${media.id}`;
  const mediaUrl = media.attachment?.url || media.fileUrl || media.thumbnailUrl;
  const mimeType = media.attachment?.mimeType || '';

  const propertyName = media.property?.name || media.propertyName || `Property #${media.propertyId}`;
  const parish = media.property?.parish || media.parish || '';
  const reportTitle = media.report?.title || media.reportName || `Report #${media.reportId}`;

  const isImage = mimeType.startsWith('image/') || media.type === 'PHOTO' || media.type === 'Image';
  const isVideo = mimeType.startsWith('video/') || media.type === 'VIDEO' || media.type === 'Video';

  const displayDate = media.uploadDate || media.createdAt ? formatDate(media.uploadDate || media.createdAt!) : 'N/A';
    ...
    <div className="bg-dark-500 rounded-lg overflow-hidden border border-dark-400 group flex flex-col justify-between">
      <div>
        ...
        <div className="p-3.5 space-y-1.5">
          <p className="text-sm text-white font-semibold truncate" title={fileName}>
            {fileName}
          </p>
          ...
          <div className="flex items-center gap-1.5 text-xs text-dark-200">
            ...
            <span className="truncate" title={reportTitle}>
              Report: {reportTitle}
            </span>
          </div>
        </div>
      ...
      <div className="px-3.5 pb-3 pt-1 border-t border-dark-400/50 flex items-center justify-between text-[11px] text-dark-200">
        <span className="uppercase font-medium text-amber-400/80">{media.type}</span>
